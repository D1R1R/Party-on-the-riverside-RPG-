...
class Player(pygame.sprite.Sprite):
    ...
    move, atk, defend
    """
    def __init__(self, pos: tuple, screen,  file_name: str, *group: any) -> None:  # noqa: E501
        super().__init__(*group)

        self._sp = Support()
        self._sc = screen

        self.image = self._sp.load_image(
            'player_animations//leaf', "idle", file_name)
        ...
        for animation_name in move_list:
            self._sp.import_animation(
                'player_animations//leaf',
                animation_name)
        # specifications
        self._act = False
        self._hp = 10
    ...
    def player_move(self) -> None:
        ...
        if (
            not keys[pygame.K_w] and not keys[pygame.K_d]
